...
import com.neumontmc.api.Models.DeserializedJsonNodes;
...
public class JSONBefhelsterung {
    ...
    protected ArrayList<String> mapSerialUserStatsToDataBlocks(JSONObject jsonObject) throws JsonProcessingException {
        ...
        JSONArray tmpArr = new JSONArray(deserializedNodes.getStats().toString());
//        tmpArr.iterator().forEachRemaining(element -> {
//            dataBlocks.add(element.toString());
//        });
        for(int i = 0; i < tmpArr.length(); i++){
            dataBlocks.add(tmpArr.get(i).toString());
            System.out.println(tmpArr.get(i).toString());
        }
        return dataBlocks;
    }
